    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const minutes = (currentTime - time)/60;
                if (minutes < 5) {
    ...
    getBlockByHash(hash) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (blocks.length)
                ...
                reject(new Error('Block with the hash ' + hash + ' not found'));
        });
    }
        ...
        return new Promise((resolve, reject) => {
            try {
                self.chain.forEach(async (block,index,array) => {
                    if (index > 0) {    // By pass the Genesis block
                        const decodedDataObj = await block.getBData()
                        const dataAddress = decodedDataObj.message.split(':')[0];
                        if (dataAddress == address) {
                            let star = {}
                            star.owner = address;
                            star.star = decodedDataObj.star;
                            stars.push(star);
                        }
                    }
                })
            ...
            self.chain.forEach(async (block,index,array) => {
                ...
                if (block.previousBlockHash != null) {
                    try {
                        ...
                        const previousBlockHash = SHA256(JSON.stringify(previousBlock)).toString();
                        if (previousBlockHash != block.previousBlockHash) {
                            errorLog.push('Unmatching hash for ' + block.previousBlockHash);
                        }
                    } catch(error) {
                        errorLog.push(error);
                    }
